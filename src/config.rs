...
impl DplConfig {
    ...
    fn load_config(config_file_path: &PathBuf) -> Option<toml::Table> {
...
pub fn update_config(config_file_path: &PathBuf, key: &str, value: &str) -> Result<()> {
    let toml_content = DplConfig::load_config_as_str(config_file_path);
    let mut doc = toml_content.unwrap_or_default().parse::<Document>()?;
    doc[key] = toml_edit::value(value);
    File::create(config_file_path).and_then(|mut file| file.write_all(doc.to_string().as_ref()))?;
